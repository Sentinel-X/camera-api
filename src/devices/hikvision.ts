...
export class HikvisionDevice extends BaseDevice implements Device {
  ...
  async getInvasionAreaCoordinates(): Promise<InvasionAreaCoordinate[]> {
    ...
    const regionWithArea = fieldDetectionRegions.find((region: any) => {
      let coordinates = region?.RegionCoordinatesList?.RegionCoordinates ?? [];
    ...
    let cameraCoordinates = regionWithArea?.RegionCoordinatesList?.RegionCoordinates ?? [];
    if (!Array.isArray(cameraCoordinates)) {
    ...
    const drawingScreenWidth = Number(fieldDetectionConfig.FieldDetection?.normalizedScreenSize?.normalizedScreenWidth) ?? 1000;
    const drawingScreenHeight = Number(fieldDetectionConfig.FieldDetection?.normalizedScreenSize?.normalizedScreenHeight) ?? 1000;
